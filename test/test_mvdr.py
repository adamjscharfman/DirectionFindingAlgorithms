...
# Test MVDR Implementation
mvdr_metric = mvdr.compute_mvdr_metric_inverse_method(rx_signal,steering_vectors,f_ind)
az_ind_est,el_ind_est,peak_val = general_df.find_argmax(mvdr_metric,num_az,num_el)

# Plot Results
az_mesh, el_mesh = np.meshgrid(az_range, el_range, indexing="ij")
plt.figure()
plt.pcolormesh(np.degrees(az_mesh),np.degrees(el_mesh),20*np.log10(mvdr_metric),shading='auto')
